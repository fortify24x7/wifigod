...
import threading
import requests
import pip #Thanks to @_sanduuz_ on instagram for PIP usage instead of subprocess
import scapy
try:
	import dns
...
#. call Error discovered by the same guy who discovered the other googolplex error that occurs when script is run on a Raspberry PI"
	...
	requests.post("http://thewifigodproject-ratings.dx.am/data.php",data=payload)
...
while True:
	...
	print(str(c.b+'7'+c.w+'.)'+c.rb+' Intercept HTTP headers (For use with #5)'+c.d))
	print(str(c.b+'8'+c.w+'.)'+c.rb+' SYN Packet Injection Overflow'))
